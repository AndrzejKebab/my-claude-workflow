...
The path uses the package's `displayName` and the sample's `displayName` from `package.json`, not the `Samples~/` folder names. Confirm the imported path from this project's package manifest rather than assuming it matches the source folder name.
...
The two trees are not auto-synced: a file may exist in the working copy but
not the distributable sample, and copying in either direction reconciles only
the files it is told to touch.
...
Verify the source and working-copy `.meta` files have matching GUIDs before
publishing, so serialized references resolve for consumers as well as locally.
...
|------|----------------------------------|
| Import location | Check `displayName` and `samples[].path` in the package's `package.json`, then import the sample. |
| Additive re-import and publishing | Compare the two trees after renaming or removing a sample file. |
| GUID preservation | Compare matching `.meta` files in the source and working-copy trees. |
| Why a GUID mismatch is a missing script | See [`monobehaviour-files.md`](monobehaviour-files.md). |
