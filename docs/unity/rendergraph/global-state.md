...
4. For **compute** passes on Vulkan: the matrix array set via `cmd.SetGlobalMatrixArray` IS visible to compute kernels, **provided** the compute dispatch is recorded into the same CommandBuffer downstream of the `SetGlobalMatrixArray` call. This is the URP `MainLightShadowCasterPass` → atmospherics-fog-populate flow: URP issues all the cascade globals via `cmd.SetGlobalVector(_CascadeShadowSplitSpheres0..3, ...)` etc. inside its render func; the fog populate pass schedules later (because it `UseTexture(mainShadow, Read)`-ed it) and its `cmd.DispatchCompute` is recorded after. URP's bindings reach the kernel.
...
What you DO need to mirror per-dispatch is anything published only via host-side `Shader.SetGlobal*` (e.g. atmospherics PhysicalSky sun/moon vectors, project-internal HFFalloff vectors). See `VolumetricFogPass.cs:1100-1115` for the canonical pattern.
...
4. **Avoid the screen-space branch**: `Shadows.hlsl:378-381` short-circuits to `SampleScreenSpaceShadowmap` when `_MAIN_LIGHT_SHADOWS_SCREEN` is on, and that branch uses implicit-LOD `Texture2D.Sample` — illegal in compute on Vulkan. The project's compute helper `ZoriComputeSampleMainLightShadow` (`Packages/is.zori.atmospherics/Runtime/VolumetricFog/Shaders/ZoriVolumetricFogPopulate.compute:66-91`) bypasses `MainLightRealtimeShadow` entirely and calls `SAMPLE_TEXTURE2D_SHADOW` (which is `SampleCmpLevelZero`, explicit-LOD) directly. See [shadow-sampling-from-compute.md](shadow-sampling-from-compute.md) for the full walkthrough.
