...
Current home: `/mnt/archive4/ORCA/miniheightfields_testbed/packing-streaming/is.zori.miniheightfields/` (supersedes a prior copy at `.../woweyreey/Packages/is.zori.heightfields/`; per the project owner the rendering approach carried over largely unchanged — what moved is the shader path, now a dedicated ShaderGraph Target). `swordgal` itself does not have this package installed; these citations are cross-project.
...
3. **Even the mesh that does exist (`HeightfieldPatchMesh.cs`) wouldn't help.** It's a small, flat, undisplaced patch template (`new Mesh` + `SetIndices(..., MeshTopology.Triangles, 0)`) — the actual terrain shape comes from per-vertex displacement in the vertex shader (`HeightfieldsSG.hlsl` / the "Heightfields Vertex Setup" subgraph), sampling a height texture/StructuredBuffer at draw time. `GeometryPool` only reads a mesh's *stored* vertex buffer — it never runs a vertex shader. This is a general Unity ray-tracing constraint (vertex/pixel-shader displacement and tessellation never reach a BLAS unless the displaced result is baked into its own buffer first), not something specific to Surface Cache.
4. **The production terrain material has no Meta pass.** The custom ShaderGraph Target (`Runtime/URP/ShaderGraphTarget/HeightfieldStampTarget.cs`) emits exactly two SubShaders — `PrimaryDependency` and `StampBasemap` — no Meta SubShader. Even if findings 1-3 were solved, this material would hit the no-Meta-pass branch and get silently replaced by the flat fallback material.

Net: **Surface Cache GI is currently blind to the heightfield terrain** — no shadow cast into the cache, no light bounce off it, no occlusion. Silently: no error, no warning, it simply never registers.

## 4. Options if GI interaction with the terrain is wanted

Ranked by how much of the existing architecture stays intact:

1. **GI-proxy mesh (recommended starting point).** A coarse, real `MeshRenderer` + `MeshFilter` sibling whose `Mesh` is populated by a **compute bake** (not sampled at vertex-shader time — see finding 3) on a coarse cadence (LOD-root change, or every few seconds — not per-frame), using a Meta-pass-carrying material (likely a second, simplified material distinct from `HeightfieldStampTarget`'s production shader, purely for the GI albedo/emission read). Given the copy-on-register semantics in §2, this proxy must force a `MeshRenderer.enabled` off/on toggle (or destroy/recreate) whenever its baked geometry changes meaningfully — neither in-place vertex mutation nor a `MeshFilter.sharedMesh` swap is picked up. Validate that refresh mechanism by sabotage test before relying on it.
