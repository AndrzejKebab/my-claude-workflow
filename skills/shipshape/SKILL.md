...
- `tools/` — self-contained instrumentation (no project files; network only where noted): `shipshape-metrics.py`, `shipshape-tells.sh`, `shipshape-scorecard.sh`, `shipshape-asmdef-graph.py`, `shipshape-fmt-suspects.py` (csharpier-flattened structured literals), and `shipshape-clones.py` (near-duplicate clone-family discovery for the E pass, via `npx -y jscpd` — the one networked tool; C#, Rust, TypeScript, and more).
...
- **M — mechanical:** formatter runs, enforcement-file commits, deterministic sweeps (deletable trailing `//` markers, emoji, dead usings), and extraction of *byte-exact* duplicates. Near-zero judgment.
...
- **S — structural:** method decomposition, type moves, asmdef splits. Requires a G2+ gate (hard rule 2).
...
**Discover.** `tools/shipshape-clones.py` ranks candidate clone families by drift-risk (instances × size). It nominates on mechanical similarity only and over-reports by design; it decides nothing.
...
6. **No boundary it should not cross** — the shared block lands in an assembly all instances already reference without a new asmdef cycle (H4), and does not couple modules kept independent on purpose.
...
- S4 Duplication % ↓ (`shipshape-clones.py` / jscpd), every extraction cleared by the six-test discriminator (the commonality pass) and refuted by the adversary before it runs — a god-helper carrying a flag per difference is worse than the honest clones, so the pass defaults to leaving a family duplicated and the rejected-merge log records why.
