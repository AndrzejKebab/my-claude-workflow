...
**`llm_request_count: 0` is NOT a failure signal on the Claude backend — never block on it.** `marker/services/claude.py` never calls `block.update_metadata(llm_request_count=…, llm_tokens_used=…)`, unlike `gemini.py` (see its `update_metadata` call), `openai.py`, `azure_openai.py`, and `ollama.py`, which all do. The counter is therefore **structurally blind for the default backend** and reads `llm_request_count: 0` / `llm_token_count: 0` in `marker-meta.json` even on a fully successful, fully-billed LLM run. Since `claude` is the default provider, that zero is the *normal* reading and says nothing about LLM health. Judge the LLM tier by evidence that actually discriminates:

- **The run log** — presence or absence of 400s / 401s, and whether per-processor steps show realistic multi-second timings (real network round-trips) rather than instant skips.
...
| `research-refiner` | **`"sonnet"`** | **Quality-control gate, and — when Pass 2 is skipped — the inline vision pass.** Refiner is where math correctness is verified, OCR-corrupted equations are reconstructed, and scientifically-load-bearing formulas (HG, Rayleigh, Mie, transport equations) get their final form before the document becomes citable. A wrong-but-plausible LaTeX equation that ships through is harder to detect than a missing one — for example, marker once produced `(1 + cos a)` for the Rayleigh phase function where the canonical form is `(1 + cos²a)`; a careless refiner pass can miss a dropped exponent because the broken form is syntactically valid LaTeX, even though the surrounding paragraph ("forward = backward scatter") only makes sense for the squared form. This is why the refiner brief must explicitly call out cross-checking equations against their surrounding prose rather than trusting syntactic validity alone. **Every pass in this pipeline — extractor, vision, and refiner — runs on Sonnet; Opus is never dispatched.** |

Concretely: every dispatch — extractor, vision, and refiner — passes `model: "sonnet"`. The orchestrator itself inherits whatever model the parent session runs on — that is fine and unavoidable; only the dispatched sub-agents need the explicit override. If you find yourself dispatching a `/research` sub-agent without a `model` argument, that is a bug — add it. If you find yourself dispatching one with `model: "opus"`, that is also a bug — this pipeline is Sonnet-only by policy.

**Math-heavy papers — recommend a manual orchestrator vision-pass audit after the refiner**: when the source paper carries scientifically load-bearing equations (radiative-transfer integrals, BRDF formulas, phase functions, error metrics, transport equations) AND the source is photoscanned with Acrobat OCR, the orchestrator should do a quick equation-by-equation cross-reference against the page renders before finishing the run. The refiner agent does this against text-layer artefacts, but a second pass by the orchestrator with direct visual access to the page renders is the right belt-and-braces approach for canonical primary sources. Note the audit in your final summary to the user.
...
- **Pass 3 (refiner)** — resolves the **content** every `FIXME(extract)` / `FIXME(vision)` mark flags. It leaves the comments in place (no hand-deletion); the **Pass 3.7 mechanical sweep** (`strip_html_comments.py`) removes every marker at once. Anything the refiner cannot resolve from text-layer + render evidence alone it surfaces **in its return message** for an orchestrator audit — NOT as an inline `<!-- FIXME(audit) -->`, which the sweep would silently erase.

**Markers are scaffolding, not deliverable — the finished document carries zero HTML comments.** All four `FIXME(*)` / `vision:` / `vision-skip` markers exist only to pass information between passes; none belong in the shipped `<slug>.md`. Removing them is a deterministic regex transform, so it is done mechanically at close-out (Pass 3.7) rather than in an agent's Edit loop. Auditability is not lost: every vision reconstruction keeps its `![sNNN-slide.png]` frame embedded directly below it, so "verify this reconstruction against the image" still works without the anchor comment — and the pre-sweep `.bak` + git history preserve the markers if a later audit ever wants them.

**Why inline instead of sidecar files:** the mark lives exactly where the problem is, so the fixing pass sees it in context while reading the document it is already reading end-to-end — no second file to open, no template ceremony, no findings content passing through the orchestrator's context. The orchestrator counts marks with a single grep, never by reading them.
...
- Immediately **above** each reconstructed region, place a greppable provenance marker: `<!-- vision: reconstructed from frame-XXXX.jpg — verify against image -->`. It anchors the audit through Pass 3 and is then removed by the Pass 3.7 sweep; the embedded frame (below) keeps the reconstruction auditable afterward.
- Embed the frame render **directly below** the reconstruction — it is the ground truth a reader/refiner checks the bullets, tables, and diagrams against, and the reason stripping the provenance comment at close-out loses no auditability.
...
- `research-refiner` may fix an obviously-wrong reconstruction against the frame, but anything it cannot verify it escalates in its **return message** (not an inline `<!-- FIXME(audit) -->`, which the Pass 3.7 sweep would erase) rather than guessing.
...
**Two ways a chart read goes wrong that "mark it ≈" does not cover.** Both were caught only by a refiner re-reading the render; both had produced confident, plausible output.

- **Decimal-comma locales.** A thesis or paper from a German/Austrian/French source draws its axes with `,` as the *decimal* separator and no thousands separator. `0 / 2,250 / 4,500 / 6,750 / 9,000` is a 0–9 axis in 2.25 steps, not 0–9000. Misreading it inflates every value 1000× — and then, because the surrounding prose no longer reconciles, the agent invents a hypothesis to explain the gap ("the axis label must be wrong, these must be microseconds") and marks *the source* as internally inconsistent. Observed 2026-07-21 on Mayer 2010, where the "unit error" was entirely in the reading. **Before concluding a source contradicts itself, check the decimal separator against a known-magnitude figure elsewhere in the same document.** A source-inconsistency claim is an extraordinary claim; the reading is far likelier to be wrong than the paper.
- **Log-scale plots read by eye.** "Approximate" reads off an unlabelled log axis are not approximate, they are wrong by *decades* — one paper's series were off by 1–2 orders of magnitude (≈2×10² recorded where the true values were 1.0×10⁴ / 1.1×10³ / 1.3×10²). Eyeballing interpolates linearly inside a decade and loses the decade itself. **Calibrate against the gridlines: locate two decade lines in pixels, then map each point's pixel offset through the log scale.** State the residual honestly afterwards ("exact decade, ±15% mantissa") rather than a bare ≈.
...
- **GPU memory pressure is a hard failure, never a CPU downgrade** (marker-GPU-only policy, user 2026-05-30). `marker_extract._require_marker_gpu` runs before torch imports and **raises** unless nvidia-smi reports ≥ 3072 MiB free; the policy comment states ~15 GiB is *expected* free, so a shortfall means something is wrong the operator should see and fix. An explicitly-empty `CUDA_VISIBLE_DEVICES` is **rejected** (`"marker-GPU-only mode forbids CPU extraction"`); a non-empty value is honoured so you can pin a GPU index. There is no `_maybe_force_cpu_inference` any more — the old silent CPU downgrade was removed precisely because it read downstream as "marker worked but produced poor output".
- **There is no PyMuPDF fall-through.** `extract_research.py` refuses it: `"marker-GPU-only mode forbids the PyMuPDF span-walker fallback"`. A marker failure is terminal for that paper — no degraded output is ever written.
- **The gate samples free VRAM once, so a batch can pass it and OOM later.** On a workstation with a live Unity editor plus Electron GPU processes (~6 GiB steady, spiking higher), paper 1 succeeds and paper 4 dies with `torch.OutOfMemoryError` mid-run. **Do not respond by forcing CPU — that is the one thing the policy forbids.** Make the *runner* resilient instead: before each paper, poll `nvidia-smi --query-gpu=memory.free` until it clears a bar well above the 3 GiB gate (~6 GiB), and retry a paper that OOMs rather than continuing past it. Per-paper `assets/<slug>/marker.md` caching makes a resumed batch cheap, and a runner that skips slugs whose `<slug>.md` already exists avoids the `.regen` sidecar (which reads as success).
...
**Symptom**: pass 1 finishes with a number of `frame-XXXX-NNNN.jpg` files much smaller than the slide count visible in the deck, or with sections whose duration runs to many minutes (a `*[MM:SS] (476s)*` header is a blob hiding a dozen slides).

Three distinct causes produce that symptom, and they have different fixes:

- **Codec (handled automatically).** OpenCV's FFmpeg backend cannot decode AV1 (YouTube's default codec) or, often, VP9 — it returns all-black frames, so the frame diff sees no change and detection collapses to a *single* scene. `research_video.py` now prefers an H.264 (`avc1`) download and, via `_ensure_cv2_decodable`, transcodes any non-H.264 source to a scratch `<stem>-h264.mp4` before scene detection; `redetect_scenes.py` does the same. A "1 scene detected" result is the fingerprint of this case and should no longer occur — if it does, check the codec with `ffprobe -show_entries stream=codec_name`.
- **The wrong metric (fixed at the source — do not tune around it).** Detection used to compare a hue-saturation histogram, which *discards luminance by construction*, so a deck of black-text-on-white slides was very nearly invisible to it. Measured on a real keynote, two entirely different text slides scored 0.0535–0.0676 while the same-slide noise floor sat at 0.0566–0.0633 — **signal below noise**, so no threshold could separate them and lowering `--threshold` only manufactured false positives elsewhere. `luma` (mean absolute difference of downscaled grayscale) separates those same pairs by ~30× and is now the default; the calibration table lives in `tools/scene_metrics.py`. Pick `--metric=hsv` only for footage where luminance changes constantly but content does not (handheld camera, stage lighting, gameplay capture), or to reproduce a pre-2026-07 extraction; `--metric=edge` keys on text/line structure and ignores flat-field brightness shifts (projector auto-exposure, fades). Because the metrics live on different scales, `--threshold` defaults to the chosen metric's calibrated value rather than one shared number.
- **Genuinely similar slides.** Consecutive slides differ only by a text line. With `luma` this mostly resolves itself; when it does not, the redetect helpers below address it.
...
It scores every pair under every metric and prints a verdict per metric — `SEPARATES (29.8x, suggest 0.0086)` means tune the threshold, `BLIND (diff falls inside same band)` means stop tuning and switch metric. One run costs about a minute and settles in one shot what a threshold sweep cannot settle at all.

There is also a *merge* over-collapse independent of detection: `process_video`'s `merge_similar_scenes` always merges consecutive same-type speaker/demo runs (and any scene < 3 s), which is right for a talking-head talk but wrong for a slide/gameplay-dense deck — it can fold 190 detected scenes back down to ~6. For those talks pass **`--no-merge`** to `research_video.py`, e.g. `research_video.py "URL" --slug=… --title=… --no-merge`, to keep one markdown section per detected slide. This replaces the older manual "bypass `process_video`, build markdown from the TSV by hand" workaround.
...
- **Prefer `--all-slides` for any talk that is slides end-to-end**, then fold. `--all-slides` treats every detected scene as a slide (OCR'd + embedded under its own `## Slide N`), which guarantees dark-themed slides are never dropped — `classify_frame`'s brightness/edge heuristic can still misclassify sparse dark slides as "speaker" and discard their frames. Its cost is that every cut to the podium camera also becomes a section, splitting one slide's narration across the slide and the podium shots that follow it. Recover the slide-centric structure afterwards with **`tools/fold_speaker_sections.py --only=<slug>`** (dry-run first), which merges speaker-only sections into the slide they narrate, renumbers, and deletes the orphaned frames. Fold *after* extraction rather than classifying during it, because the fold can use whole-document evidence the per-frame classifier cannot: podium shots come from a locked-off camera and are near-identical to *each other*, whereas every slide differs from every other slide. Do not substitute a brightness test — on a real keynote the podium frames and two genuine content slides (a wireframe head on black, a Valve Source Editor screenshot) were equally dark, and brightness alone would have silently deleted the content. Reserve the auto-classified path for genuine talking-head/mixed-footage talks.
...
After Pass 2.5 (validate) completes with errors enumerated to disk, dispatch a single `research-refiner` agent (Sonnet, `model: "sonnet"`) with a brief listing the specific concerns the orchestrator wants fixed:
...
- **A top-of-document summary is written for every paper**, and it MUST carry a **"Relevance & applications" subsection** (see "Relevance & applications analysis" below). The brief also names which internal sections / cross-references the prose summary should cover.
...
After Pass 3 returns, the orchestrator re-runs `tools/validate_research.py --only=<slug>` (Pass 3.5) as the clean-room check — see Pass 2.5 above. Once 3.5 is clean, it runs `tools/update_topics.py --only=<slug>` (Pass 3.6, the final bookkeeping step): the script reads the document's final `tags` and splices it into the `topics/<tag>.md` index pages for the bundle it lives in. Topic membership is derived from frontmatter — the script owns only the member table between the `<!-- members:start -->` / `<!-- members:end -->` markers and the topic index, preserving each page's intro — and it is concurrency-safe (a per-bundle `flock` serializes the read-derive-write and writes publish atomically via `os.replace`), so parallel extractions of different papers may run it at the same time.

**Pass 3.7 — mechanical comment sweep (final document polish).** Run `tools/strip_html_comments.py --only=<slug>` to remove every remaining `<!-- … -->` marker from `<slug>.md` so the delivered document carries zero HTML comments (see "Markers are scaffolding" above). It writes a timestamped `.prestrip-*.bak` beside the file, prints a unified diff, and self-checks that nothing but comment spans changed (refusing with exit 3 otherwise) — deterministic and recoverable, which is why it is a script and not agent Edit work. **Scope discipline: it runs on `<slug>.md` ONLY, never on the `topics/<tag>.md` pages** — those depend on their `<!-- members:start/end -->` markers, which `--only=<slug>` never touches (it resolves to `Prepared/<slug>.md`). Order vs 3.6 is free (the sweep touches only body comments, not the frontmatter `tags` update_topics reads); running it last as the final polish is the convention. Capture the refiner's return-message escalations BEFORE deleting the `.bak`, then remove the `.bak` once you have eyeballed the diff.

### Relevance & applications analysis (standing regimen — every paper)

Every extracted paper carries, inside its top-of-document summary block, a short **"Relevance & applications"** analysis: *what the paper is in two or three sentences, then how a practitioner could apply it to optimise a real system.* The refiner writes it into the document; the orchestrator additionally **proposes the same analysis to the user in its final message** and offers to go deeper. This is not decorative colour — it is the bridge from "citable artifact on disk" to "thing someone builds with", and it is the single most-reused part of an extraction for an engineer scanning the corpus.

Discipline (binding):

- **Enumerate the option space; do not pick a pole.** Where the technique helps, where it does NOT, the most promising angle, and honest caveats — the differentiated-then-integrated shape (per `NONDUAL.md`), never a yes/no verdict. Sanity check: if a reader could not tell *when* to reach for the technique versus when to leave it, the analysis has not done its job.
- **Separate paper claims from applied speculation.** State plainly which mappings the paper itself supports and which are your extrapolation; an applied guess must never read as a source claim.
- **Decouple from the invoking project.** `/research` is invoked from whatever repo the user happens to be in; the corpus is cross-project and that repo may be entirely unrelated to the paper. The recorded analysis names domains and system classes generically (e.g. "large-world streaming renderers", "asset-import / voxelization pipelines"), never "your current project" or a repo-specific subsystem. If the user framed a project-specific question in chat, answer *that* in chat — but what lands in the document's summary stays general and portable.
- **Ground it in the paper's actual mechanism**, not the marketing title: name the load-bearing property (a true distance vs a mere inside/outside indicator, an analytic vs a discretised representation, a feed-forward fit vs a global solve) and which application each property unlocks. The mapping is only as trustworthy as the mechanism it rests on.

When 3.5 is clean, the topic pages are updated, and `grep -cF '<!--' <slug>.md` is **0**, the run is done; summarise the work in your final message to the user — including the Relevance & applications analysis (see above) — and note any items the refiner escalated for audit (they live in its return message, no longer inline).
...
|--------|---------|---------------|
| `tools/research_video.py` | YouTube or local video → scene detection, OCR, transcript alignment. Accepts `--title=` `--slug=` `--metric=` `--threshold=` `--no-merge` `--all-slides` flags. `--metric=luma\|hsv\|edge` picks the frame-difference measure (default `luma`; see `tools/scene_metrics.py`) and `--threshold=` defaults to that metric's calibrated value. `--all-slides` treats every detected scene as a slide (no brightness/edge classification) — use it for slide-end-to-end talks so dark-themed slides aren't dropped (see "Slide-centric emit & dark-slide handling"). Emit is slide-centric: speaker narration folds into the current slide, never a bare `## [MM:SS]` header per micro-scene. Transcript comes from the SOTA STT pass: an SRT already next to the mp4 (same stem, `.en.srt`) is reused, otherwise the audio is re-transcribed via `transcribe_to_srt.transcribe()`. YouTube auto-captions are never used. | Refuses to overwrite existing `<slug>.md` — writes `<slug>.regen-<YYYYMMDD-HHMMSS>-<6hex>.md` next to it instead (randomised so concurrent agents don't clobber each other). Pass `--force` to overwrite in place. |
| `tools/redetect_scenes.py` | Aggressive scene re-detection for slide-heavy talks (finer interval, selectable `--metric`/`--threshold`). Writes `scene-NNN-*.jpg` to the asset dir + a TSV to `/tmp`. | Append-only. Manually clear stale `scene-*.jpg` first if you re-run with different parameters. |
| `tools/scene_metrics.py` | Shared frame-difference metrics (`luma`, `hsv`, `edge`) plus their calibrated default thresholds, imported by `research_video.py` and `redetect_scenes.py` so the two cannot drift. Carries the measurement showing the legacy hue-saturation histogram is blind to text-only slide changes. Not invoked directly. | Pure library, no file writes. |
| `tools/probe_scene_metrics.py` | Diagnoses a under-segmenting detector: given `--same`/`--diff` timestamp pairs you have eyeballed, scores them under every metric and prints `SEPARATES (Nx, suggest T)` or `BLIND (diff falls inside same band)` per metric. Use it instead of a threshold sweep the moment detection looks wrong — it distinguishes a mistuned threshold from a metric that cannot see the change at all. Exits 1 if every metric is blind. | Read-only on the video, no file writes. |
| `tools/fold_speaker_sections.py` | Post-pass for `--all-slides` extractions: finds the largest cluster of mutually-similar frames (the locked-off podium camera), folds those sections' transcript into the slide they narrate, renumbers sections, and deletes the orphaned frames. `--only=<slug>`, `--dry-run`, `--threshold`, `--min-cluster`, `--keep-frames`. Folds nothing when no such cluster exists. | Rewrites `<slug>.md` in place and deletes folded frames — **always `--dry-run` first**. Transcript text is preserved verbatim (word count is unchanged by the fold). |
...
| `tools/align_video_to_deck.py` | **Dual-source alignment** (deck + recording of the same talk): matches each sampled recording frame against the deck's own `sNNN-slide.png` renders to decide which slide is on screen when, then cuts the SRT into per-slide windows. Chrome-cropped edge-gradient NCC (raw luminance is blind on text slides) + optional OCR token signal (`--pdf`) + monotonic DP (slide index non-decreasing in time). Auto-detects the slide pane or takes `--rect x,y,w,h`. `--control shuffle` is the sabotage check (permuting the deck must collapse the score ~10×); `--from-tsv` re-emits windows without recomputing the frame scoring; `--emit-windows`/`--pad-slides` write one positional block per slide for `fold_narration.py`. See "Dual source" under Determining Input Type. | Writes an alignment TSV to the assets dir + the explicit `--emit-windows` path. Read-only on the recording. |
| `tools/fold_narration.py` | Folds a per-slide windows file (from `align_video_to_deck.py --emit-windows` or `srt_to_windows.py`) into a slide-deck `<slug>.md`, inserting each slide's narration as a `> **<label>:**` blockquote immediately after that slide's `![sNNN-slide.png]` image. Positional: requires one window block per `## Slide N` section. `--label`, `--wrap`. | Edits `<slug>.md` in place; refuses to run twice (guards on the label) so a re-run can't double the narration. Run only when no vision/refine agent is editing the same file. |
...
When the arguments give **two sources that are the same talk** — a published slide PDF/PPTX *and* a recording of it being delivered (GDC Vault HLS, a conference YouTube upload, a local mp4) — do NOT make two half-documents. Build **one** document keyed on the deck (the clean, high-resolution source of every slide's structure) and fold the speaker's narration in as the per-slide `> **Narration …:**` blockquote. That is the canonical section shape, and it is strictly better than either source alone: the deck gives crisp renders and text; the recording gives what the slides never say out loud.

The load-bearing problem is **alignment** — which slide is on screen when the speaker says a given sentence. Solve it by matching the recording against the deck's own renders (an analytical oracle: the answer is checkable, not guessed), NOT by time-proportional guessing (the fallback in `align_video_to_slides.py`, ±1–3 slides and worse when the talk dwells unevenly).

**Flow** (after Pass 1 has produced the 107-ish `sNNN-slide.png` deck renders and you have the recording + its SRT):

1. **Detect the slide pane.** Conference recordings are usually a fixed composite (speaker camera beside a slide pane, sometimes cutting to full-screen slide for embedded video). `align_video_to_deck.py` auto-detects the pane by template-matching a few deck renders, or pass `--rect x,y,w,h` if you've measured it. Sample a handful of frames first (`ffmpeg -ss`) to see the layout.
2. **Align.** `align_video_to_deck.py <recording> --slug=<slug> --pdf=<deck.pdf> [--rect …]` scores each sampled frame against every deck render (chrome-cropped edge-gradient NCC — raw luminance is **blind** on black-text-on-white slides; the OCR `--pdf` token signal pins the demo-heavy back half where neighbouring "label over gameplay" slides drift), then runs a monotonic DP so the slide index is non-decreasing in time. Writes an alignment TSV to the assets dir.
3. **VALIDATE before trusting — this is not optional.** A green alignment is a claim about the fixture. Confirm it can go red: (a) hand-verify ≥2 anchors — grep a distinctive slide phrase to get its true number, read the frame at a timestamp inside its window, confirm they match; (b) run the sabotage control `--control shuffle` — permuting the deck MUST collapse the median matched path score (measured ~1.29 → 0.13, a 10× drop). If shuffling does **not** collapse the score, the metric is reading layout, not slide identity, and nothing downstream is trustworthy — fix the metric, do not proceed.
