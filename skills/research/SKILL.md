...
**If you find yourself lowering `--threshold` repeatedly and still seeing multi-minute sections, stop — that is the signature of a blind metric, not a mistuned one.** Confirm it the cheap way before touching anything: grab two frames you know are different slides plus two you know are the same slide, score both pairs, and check whether the "different" score clears the "same" band at all. If it doesn't, change the metric.
