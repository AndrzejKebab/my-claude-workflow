...
When 3.5 is clean, the topic pages are updated, and `grep -cF '<!--' <slug>.md` is **0**, the run is done; summarise the work in your final message to the user (note any items the refiner escalated for audit — they live in its return message, no longer inline).
