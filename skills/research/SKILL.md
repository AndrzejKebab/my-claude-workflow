...
- **Resolve every inline `FIXME` mark.** The brief MUST tell the refiner to grep the document with a **literal** matcher — `grep -nF 'FIXME(extract)'` and `grep -nF 'FIXME(vision)'`, as two calls — fix each flagged item, and delete the comment once handled. Do NOT brief `grep -n 'FIXME(extract)\|FIXME(vision)'`: under the ripgrep rewrite the parens become capture groups and `\|` becomes a literal pipe, so it silently matches nothing and the refiner concludes there is no work (see the boxed warning under "Inline `FIXME` marks"). The brief must carry that warning verbatim, since a zero count looks identical to a clean document from inside the agent. Anything it cannot resolve from text-layer + render evidence alone it re-marks `<!-- FIXME(audit): … -->` and lists in its return message. A refiner that finishes with `FIXME(extract)` / `FIXME(vision)` comments still present has not completed its pass.
