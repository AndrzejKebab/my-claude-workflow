...
4. **Purpose-first principles, read by people.** Principles are compressed experience about where pain lands in living codebases — reach for whichever one explains the pain a candidate shape would cause the people who read, change, and debug it. (The session that wrote this: coupling explained why two parallel groups would collide on one settings file; information hiding is why a world→UV mapping stays internal to the sampler; domain boundaries are why per-view shadow state lives beside per-view HiZ state instead of in a task silo.) Named principles — cohesion/coupling, Parnas, SOLID, DDD — are instances of the kind, not a checklist: locking onto a fixed list reproduces the gate-serving failure one level up, with principle-compliance as the new false objective. The working test is never "which principles does this satisfy" but "what does this shape cost its next reader" — code is written for people to read and only incidentally for machines to execute, so a construct earns its place by being a usable implement: convenient to hold, clear to understand, hard to misuse. The codebase must never become a pile of ad-hoc implements that each fulfill one immediate spec and gate (user, 2026-07-15). Design reasoning is bound by `~/_dev/my-claude-workflow/NONDUAL.md`: gates, principles, and intention are lenses on one judgment, not competing poles — never pole-flip between them.
