...
9. **A completion is a claim, not proof — verify the agent actually worked.** A "completed" notification can be a no-op misfire: zero tool-uses, garbled output, nothing committed, tree unchanged. Before relaying any result as progress, confirm work happened — tool-uses > 0 **and** HEAD/tree moved (or the gate output is genuinely present). A no-op is not a result: resume that agent with an execute directive, or `TaskStop` and re-dispatch — never report it as done or mine it for a finding. The same `git status`/`git log` check catches the inverse — an agent that claims green while nothing landed.
